...
export async function futuresOhlcv(symbol: string, timeframe: string, limit: number, since?: number) {
  // For futures, ccxt expects the same 'SOL/USDT' format
  return ex.fetchOHLCV(symbol.replace('USDT', '/USDT'), timeframe, since, limit);
}
export async function markPrice(symbol: string) {
...
export async function fundingRates(symbol: string, limit: number) {
  // ccxt `fetchFundingRateHistory` varies by exchange support
  if (typeof ex.fetchFundingRateHistory !== 'function') throw new Error('Funding rate history not supported by ccxt version');
  const sym = symbol.replace('USDT','/USDT');
  const rows = await ex.fetchFundingRateHistory(sym, undefined, limit);
  return rows.map((r:any)=>({ t: r['timestamp'], rate: Number(r['fundingRate'] ?? r['info']?.fundingRate ?? 0), interval: '8h' }));
}
export async function openInterest(symbol: string, timeframe: string, limit: number) {
  // Not all OI endpoints are unified in ccxt across exchanges;
  // for demo we use fetchOpenInterestHistory if present, else throw.
  if (typeof ex.fetchOpenInterestHistory !== 'function') throw new Error('Open interest history not supported by ccxt version');
  const sym = symbol.replace('USDT','/USDT');
  const rows = await ex.fetchOpenInterestHistory(sym, timeframe, undefined, limit);
  return rows.map((r:any)=>({ t: r['timestamp'], oi: Number(r['openInterestBase'] ?? r['openInterestQuote'] ?? r['openInterest'] ?? 0) }));
}
